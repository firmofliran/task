...
public class OracleSplitPage extends HttpServlet {
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		//分页查询时的几条数据
		List<StudentRank> listsplitpage = sr.selectSplitPage(currentpage, pagesize);
		//将所有排名信息里面有名次的和当前分页查看的信息比对，符合的筛选出来
		List<StudentRank> finallylist = new ArrayList<>();
		//排序过后的所有学生信息
		List<StudentRank> all = sr.selectRank();
		for (StudentRank stu : listsplitpage) {
			for (StudentRank studentRank : all) {
				if(stu.getStuno().equals(studentRank.getStuno())){
